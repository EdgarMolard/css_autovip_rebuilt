<?php
	...
	*/
	...
	else
	{
		...
		$access_vip = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM `".SQL_PREFIX."_droits` WHERE `id` ='".$_SESSION['af_id']."' AND `type_droit`='1'"));
		$access_admin = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM `".SQL_PREFIX."_droits` WHERE `user_id`= '".$_SESSION['af_id']."' AND `type_droit`='2'"));
		...
		if ($_GET['t'] > 0 && $_GET['t'] < 5 && is_numeric($_GET['server_id']))
		{
			...
			if ($type == 4)	{$droit_name = "3Jours de Test";$champs = "vip_prix";}
			if ($type != 3)
			{
			...
			elseif ($type == 1)
			{
				...
				if ($new)
				{
					$query = "SELECT user_id FROM af_droits WHERE user_id = '".$_SESSION['af_id']."' AND `type_droit`='".$type."'";
			...
			{
				//On verifie si l'utilisateur prolonge, ou non
				$admin_req = mysqli_query($conn, "SELECT id, date_start, date_fin FROM `".SQL_PREFIX."_droits` WHERE `user_id`= '".$_SESSION['af_id']."' AND `type_droit`='".$type."'");
			...
			{
				//On verifie si l'utilisateur prolonge, ou non
				$vip_req = mysqli_query($conn, "SELECT id, date_start, date_fin FROM `".SQL_PREFIX."_droits` WHERE `user_id`= '".$_SESSION['af_id']."' AND `type_droit`='".$type."'");
				$data = mysqli_result($vip_req,0, 'date_fin');
				$start = mysqli_result($vip_req,0, 'date_start');
				$id_droit = mysqli_result($vip_req,0, 'id');
				if (!$start)			$start = time();
							...
							while($row = mysqli_fetch_array($requete))
							{
								...
								//$row['']
									...
									<tr>
										...
										$status_req = mysqli_query($conn, "SELECT is_suspended FROM `".SQL_PREFIX."_droits` WHERE `user_id`= '".$_SESSION['af_id']."' AND `type_droit`='2'");
										...
										{
											$vip_req = mysqli_query($conn, "SELECT date_fin FROM `".SQL_PREFIX."_droits` WHERE `user_id`= '".$_SESSION['af_id']."' AND `type_droit`='1'");
											$data = mysqli_result($vip_req,0, 'date_fin');
										...
										{
											$admin_req = mysqli_query($conn, "SELECT date_fin FROM `".SQL_PREFIX."_droits` WHERE `user_id`= '".$_SESSION['af_id']."' AND `type_droit`='2'");
											$data = mysqli_result($admin_req,0, 'date_fin');
											if ($data < $timestamp_geo && $timestamp_geo > $data)		$data = $timestamp_geo;
